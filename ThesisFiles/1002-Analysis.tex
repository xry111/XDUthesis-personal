...
其 $\ell_2$ 范数为 $1$ ，均值为 $0$ 。这是绝大多数压缩感知稀疏恢复算法对
矩阵的必要要求。 在实际测量中，我们不可能产生负的光通量，因此需要对采样数据
进行预处理，才能用恢复算法进行求解。
...
\paragraph{对边界模糊的坏块重建结果} 如图 \ref{fig:TV2dsmooth}
所示。重建结果很差，这是由于图像
...
\paragraph{对栅状电极的重建结果} 如图 \ref{fig:TVfinger}
